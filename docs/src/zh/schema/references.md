...
聚合字段会把子表行收集到父表行上。它适合这种情况：编辑时希望数据保持成多张规范化表，运行时又希望父对象直接带着嵌套值。
...
运行时可能希望 `Quest` 里直接有 `rewards: list<Reward>`。这时在 `Quest` 表上声明一个 list 聚合字段：
...
如果父表只需要一个子表值，就使用非 list 字段类型。`T` 要求恰好匹配一条子表行，`optional<T>` 允许匹配零条或一条子表行。如果匹配到多行，Sora 会报错。

如果父表应该接收子表行中的某一个字段，而不是把整行组装成 struct，可以声明 `value_field`：
...
含义是：`Event.condition` 接收 `EventConditionEntry.value`，前提是该子行的 `event_id` 等于 `Event.id`。子表里仍然可以有 `id`、`event_id`、备注、排序字段等辅助列；只有 `value_field` 会被复制到父表字段。
...
| `value_field` | 否 | 子表上的字段名。存在时，聚合复制这个字段的值，而不是从整行组装 struct。 |
| `order_by` | 否 | 子表上的字段名。存在时，匹配到的子行按这个字段升序排序。 |

`source_table`、`parent_key`、`child_key` 必须一起出现。`value_field` 和 `order_by` 只能作为聚合配置的一部分使用。只写其中一部分是不完整的聚合配置。
...
没有 `value_field` 时，聚合值类型必须是 struct，也就是 `list<struct<...>>`、`struct<...>` 或 `optional<struct<...>>`。结构体字段会从子表同名字段复制：
...
有 `value_field` 时，聚合值类型必须和该子表字段兼容。例如 `type = "union<EventCondition>"` 可以从同样是 `union<EventCondition>` 的子表字段 `value` 聚合。

聚合字段不能同时声明 `default`。它的值来自匹配到的子表行。没有匹配行时，`list<T>` 是空列表，`optional<T>` 是 `null`，`T` 会报错。
