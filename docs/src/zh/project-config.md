...
项目清单既可以只是 schema root，也可以是完整的构建描述。它可以写成 SCON、TOML、YAML、JSON 或 Lua；新项目推荐 SCON。
...
多语言通过 project root 的 `localization` 声明。keyed sources 独立于普通 table；见[多语言](localization.md)。
