// This file was generated by Sora. Do not edit manually.

package com.sora.showcase;

import java.util.List;
import java.util.Map;
import java.util.Set;

public final class CharacterTable extends java.util.AbstractMap<Integer, Character> implements SoraConfig.KeyedTable<Integer, Character> {
    public static final String NAME = "Character";
    public static final SoraConfig.TableInfo INFO = new SoraConfig.TableInfo(
        NAME,
        "Character",
        SoraConfig.TableShape.KEYED,
        new SoraConfig.KeyInfo("id", "Integer"),
        List.of(
        )
    );
    private final List<Integer> keys;
    private final java.util.Map<Integer, Character> rows;

    private CharacterTable(List<Integer> keys, java.util.Map<Integer, Character> rows) {
        this.keys = List.copyOf(keys);
        this.rows = Map.copyOf(rows);
    }

    private static CharacterTable fromRows(List<Character> rows) {
        return new CharacterTable(
            rows.stream().map(Character::id).toList(),
            SoraConfig.decodeMapTable(rows, Character::id)
        );
    }

    static CharacterTable decode(SoraTableSource source) {
        return fromRows(source.decodeTable(NAME, Character::decode, Character::decode));
    }

    public java.util.Map<Integer, Character> rows() {
        return rows;
    }
    @Override
    @SoraNullable
    public Character get(Object key) {
        return rows.get(key);
    }

    @Override
    public List<Integer> orderedKeys() {
        return keys;
    }

    @Override
    public List<Character> orderedRows() {
        return keys.stream().map(rows::get).toList();
    }

    @Override
    public Set<Map.Entry<Integer, Character>> entrySet() {
        return rows.entrySet();
    }
    @Override
    public SoraConfig.TableInfo info() {
        return INFO;
    }

    @Override
    public int size() {
        return rows.size();
    }
}
