...
import { collectMaintenanceInfoTextKeys, decodeMaintenanceInfo, decodeMaintenanceInfoValue } from "./maintenance_info.js";
...
export function collectGameSettingsTextKeys(value: GameSettings, out: TextKey[]): void {
    collectVec3TextKeys(value.spawnPos, out);
    for (const item of value.spawnPoints) { collectVec3TextKeys(item, out); }
    if (value.maintenance !== undefined) { const item = value.maintenance; collectMaintenanceInfoTextKeys(item, out); }
}
