...
export function formatText(template: string, args: Record<string, unknown>): string {
    let out = "";
    let rest = template;
    while (true) {
...
export interface SoraConfigTable<Row = unknown> extends Iterable<Row> {
    info(): SoraTableInfo;
...
export interface SoraKeyedTable<Key, Row> extends SoraConfigTable<Row> {
    ...
    readonly orderedRows: readonly Row[];
}
...
function readLengthPrefixedString(bytes: Uint8Array, cursor: number): { value: string; cursor: number } {
