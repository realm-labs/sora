// This file was generated by Sora. Do not edit manually.

#pragma once

#include "sora_runtime.hpp"
#include "event_condition.hpp"

#include <unordered_map>
#include <string>
#include <vector>

namespace sora::showcase {

struct ComplexConditionGroup {
    std::int32_t id;
    std::string name;
    // A derived list of union values; each child row is edited without JSON
    std::vector<EventCondition> conditions;

    static ComplexConditionGroup decode(SoraReader& reader) {
        return ComplexConditionGroup{
            reader.read_i32(),
            reader.read_string(),
            reader.read_vector<EventCondition>(),
        };
    }
};

class ComplexConditionGroupTable final : public SoraTable {
public:
    static constexpr const char* NAME = "ComplexConditionGroup";
    static const SoraTableInfo& table_info() {
        static const SoraTableInfo info = {
            NAME,
            "ComplexConditionGroup",
            "keyed",
            "id"
        };
        return info;
    }

    ComplexConditionGroupTable() {}
    ComplexConditionGroupTable(const ComplexConditionGroupTable&) = delete;
    ComplexConditionGroupTable& operator=(const ComplexConditionGroupTable&) = delete;
    ComplexConditionGroupTable(ComplexConditionGroupTable&&) = default;
    ComplexConditionGroupTable& operator=(ComplexConditionGroupTable&&) = default;

    static ComplexConditionGroupTable decode(const SoraBundle& bundle) {
        std::vector<ComplexConditionGroup> rows = bundle.decode_table<ComplexConditionGroup>(NAME);
        ComplexConditionGroupTable table;
        for (std::size_t index = 0; index < rows.size(); ++index) {
            const ComplexConditionGroup& row = rows[index];
            table.keys_.push_back(row.id);
            table.rows_.emplace(row.id, row);
        }
        table.build_indexes();
        return table;
    }

    const SoraTableInfo& info() const override { return table_info(); }
    std::size_t size() const override {
        return rows_.size();
    }
    const std::vector<std::int32_t>& keys() const {
        return keys_;
    }

    const ComplexConditionGroup* get(const std::int32_t& key) const {
        typename std::unordered_map<std::int32_t, ComplexConditionGroup>::const_iterator it = rows_.find(key);
        if (it == rows_.end()) {
            return nullptr;
        }
        return &it->second;
    }

    const std::unordered_map<std::int32_t, ComplexConditionGroup>& rows() const {
        return rows_;
    }

    std::vector<const ComplexConditionGroup*> ordered_rows() const {
        std::vector<const ComplexConditionGroup*> rows;
        rows.reserve(keys_.size());
        for (typename std::vector<std::int32_t>::const_iterator key = keys_.begin();
             key != keys_.end();
             ++key) {
            typename std::unordered_map<std::int32_t, ComplexConditionGroup>::const_iterator it = rows_.find(*key);
            if (it != rows_.end()) {
                rows.push_back(&it->second);
            }
        }
        return rows;
    }

private:
    void build_indexes() {
    }
    std::vector<std::int32_t> keys_;
    std::unordered_map<std::int32_t, ComplexConditionGroup> rows_;
};

} // namespace sora::showcase
