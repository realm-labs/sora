...
sora_result sora_showcase_quest_reward_decode(sora_reader* reader, sora_showcase_quest_reward* out) {
    *out = (sora_showcase_quest_reward){0};
    {
        sora_result result = sora_reader_read_i32(reader, &out->quest_id);
        if (result.code != SORA_OK) {
            sora_showcase_quest_reward_free(out);
            return result;
        }
    }
    {
...
void sora_showcase_quest_reward_free(sora_showcase_quest_reward* value) {
    ...
    *value = (sora_showcase_quest_reward){0};
}
