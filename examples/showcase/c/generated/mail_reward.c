...
static void sora_showcase_mail_reward_table_free_rows(sora_showcase_mail_reward* rows, size_t len) {
    if (rows == NULL) {
        return;
    }
    for (size_t index = 0; index < len; ++index) {
        sora_showcase_mail_reward_free(&rows[index]);
    }
    free(rows);
}
...
sora_result sora_showcase_mail_reward_table_load(
    ...
    if (result.code != SORA_OK) {
        sora_showcase_mail_reward_table_free(table);
        return result;
    }
    *out = table;
    return sora_ok();
...
void sora_showcase_mail_reward_table_free(sora_showcase_mail_reward_table* table) {
    ...
    sora_showcase_mail_reward_table_free_rows(table->rows, table->len);
    free(table);
}
