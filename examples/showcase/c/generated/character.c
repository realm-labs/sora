...
static void sora_showcase_character_table_free_rows(sora_showcase_character* rows, size_t len) {
    if (rows == NULL) {
        return;
    }
    for (size_t index = 0; index < len; ++index) {
        sora_showcase_character_free(&rows[index]);
    }
    free(rows);
}
...
sora_result sora_showcase_character_table_load(
    ...
    if (result.code != SORA_OK) {
        sora_showcase_character_table_free(table);
        return result;
    }
    *out = table;
    return sora_ok();
...
void sora_showcase_character_table_free(sora_showcase_character_table* table) {
    ...
    sora_showcase_character_table_free_rows(table->rows, table->len);
    free(table);
}
...
const sora_showcase_character* sora_showcase_character_table_get(
    ...
    for (size_t index = 0; index < table->len; ++index) {
        const sora_showcase_character* row = &table->rows[index];
        if (row->id == key) {
            return row;
        }
    }
