...
static void sora_showcase_shop_item_table_free_rows(sora_showcase_shop_item* rows, size_t len) {
    if (rows == NULL) {
        return;
    }
    for (size_t index = 0; index < len; ++index) {
        sora_showcase_shop_item_free(&rows[index]);
    }
    free(rows);
}

sora_result sora_showcase_shop_item_table_load(
    const sora_bundle* bundle,
    sora_showcase_shop_item_table** out
) {
    ...
    if (result.code != SORA_OK) {
        sora_showcase_shop_item_table_free(table);
        return result;
    }
    *out = table;
    return sora_ok();
...
void sora_showcase_shop_item_table_free(sora_showcase_shop_item_table* table) {
    if (table == NULL) {
        return;
    }
    sora_showcase_shop_item_table_free_rows(table->rows, table->len);
    free(table);
}

const sora_showcase_shop_item* sora_showcase_shop_item_table_rows(
    const sora_showcase_shop_item_table* table,
    size_t* out_len
) {
    if (out_len != NULL) {
        *out_len = table->len;
