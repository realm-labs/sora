...
static const char* sora_showcase_drop_group_table_name = "DropGroup";

struct sora_showcase_drop_group_table {
    sora_showcase_drop_group* rows;
    size_t len;
};

static void sora_showcase_drop_group_table_free_rows(sora_showcase_drop_group* rows, size_t len) {
    if (rows == NULL) {
        return;
    }
    for (size_t index = 0; index < len; ++index) {
        sora_showcase_drop_group_free(&rows[index]);
    }
    free(rows);
}
...
sora_result sora_showcase_drop_group_table_load(
    ...
    sora_showcase_drop_group_table* table = (sora_showcase_drop_group_table*)calloc(1, sizeof(sora_showcase_drop_group_table));
    if (table == NULL) {
        return sora_error(SORA_ERROR_OUT_OF_MEMORY, "failed to allocate table");
    }
    sora_result result = sora_bundle_decode_table(
        bundle,
        sora_showcase_drop_group_table_name,
    ...
    if (result.code != SORA_OK) {
        sora_showcase_drop_group_table_free(table);
        return result;
    }
    *out = table;
    return sora_ok();
...
void sora_showcase_drop_group_table_free(sora_showcase_drop_group_table* table) {
    ...
    sora_showcase_drop_group_table_free_rows(table->rows, table->len);
    free(table);
}
...
const sora_showcase_drop_group* sora_showcase_drop_group_table_get(
    ...
    for (size_t index = 0; index < table->len; ++index) {
        const sora_showcase_drop_group* row = &table->rows[index];
        if (row->id == key) {
            return row;
        }
    }
