...
---@field value string
local TextKey = {}
TextKey.__index = TextKey

---@param value string
---@return TextKey
function Runtime.new_text_key(value)
    return setmetatable({ value = value }, TextKey)
end

---@return string
function TextKey:as_string()
    return self.value
end

local SORA_BUNDLE_VERSION = 3
...
---@param bytes string
---@return LocalePack
function Runtime.parse_locale_pack(bytes)
    if #bytes < 8 or bytes:sub(1, 4) ~= "SORI" then
        error("invalid Sora i18n pack magic")
    end
    local version = read_u32_at(bytes, 4)
    if version ~= 1 then
        error("unsupported Sora i18n pack version " .. tostring(version))
    end
    local cursor = 8
    local schema_fingerprint
    schema_fingerprint, cursor = read_len_prefixed_string(bytes, cursor)
    local locale
    locale, cursor = read_len_prefixed_string(bytes, cursor)
    local count = read_u32_at(bytes, cursor)
    cursor = cursor + 4
    local translations = {}
    for _ = 1, count do
        local key
    ...
    if cursor ~= #bytes then
        error("Sora i18n pack has trailing bytes")
    end
    return {
...
---@class SoraI18n
---@field private expected_fingerprint string
---@field private locales table<string, boolean>
---@field private active_locale string
---@field private packs table<string, LocalePack>
local SoraI18n = {}
SoraI18n.__index = SoraI18n

---@param expected_fingerprint string
---@param locales string[]
---@param default_locale string
---@return SoraI18n
function Runtime.new_i18n(expected_fingerprint, locales, default_locale)
    local locale_set = {}
    for _, locale in ipairs(locales) do
        locale_set[locale] = true
    end
    return setmetatable({
...
---@param locale string
function SoraI18n:set_locale(locale)
    if not self.locales[locale] then
        error("unknown locale `" .. tostring(locale) .. "`")
    end
    if self.packs[locale] == nil then
        error("locale `" .. tostring(locale) .. "` is not mounted")
    end
...
---@param key string
---@return string
function SoraI18n:text(key)
    if type(key) == "table" and key.value ~= nil then
        key = key.value
    end
    local pack = self.packs[self.active_locale]
    if pack == nil then
        error("locale `" .. tostring(self.active_locale) .. "` is not mounted")
    end
    local value = pack.translations[key]
    if value == nil then
...
function read_len_prefixed_string(bytes, cursor)
    local len = read_u32_at(bytes, cursor)
    cursor = cursor + 4
    local finish = cursor + len
    if finish > #bytes then
        error("Sora i18n string exceeds pack length")
    end
    return bytes:sub(cursor + 1, finish), finish
end
