%% This file was generated by Sora. Do not edit manually.

-module(complex_condition_group_entry).
-export([decode/1, decode_value/1, decode_table/1, rows/1
    , get/2, keys/1, ordered_rows/1
]).
-export_type([t/0, table/0]).

-type t() :: #{
    'id' := integer(),
    'group_id' := integer(),
    'seq' := integer(),
    'value' := event_condition:t()
}.

-type table() :: map().
-define(TABLE_NAME, <<"ComplexConditionGroupEntry">>).

-spec decode(sora_runtime:reader()) -> {t(), sora_runtime:reader()}.
decode(Reader0) ->
    {Id, Reader1} = (fun sora_runtime:read_i32/1)(Reader0),
    {GroupId, Reader2} = (fun sora_runtime:read_i32/1)(Reader1),
    {Seq, Reader3} = (fun sora_runtime:read_i32/1)(Reader2),
    {Value, Reader4} = (fun event_condition:decode/1)(Reader3),
    {#{
        'id' => Id,
        'group_id' => GroupId,
        'seq' => Seq,
        'value' => Value
    }, Reader4}.

-spec decode_value(map()) -> t().
decode_value(Value) ->
    Obj = sora_runtime:expect_map(Value),
    #{
        'id' => sora_runtime:expect_integer(sora_runtime:value_get(<<"id">>, Obj)),
        'group_id' => sora_runtime:expect_integer(sora_runtime:value_get(<<"group_id">>, Obj)),
        'seq' => sora_runtime:expect_integer(sora_runtime:value_get(<<"seq">>, Obj)),
        'value' => event_condition:decode_value(sora_runtime:value_get(<<"value">>, Obj))
    }.

-spec decode_table(map()) -> table().
decode_table(Bundle) ->
    Rows = sora_runtime:decode_table(Bundle, ?TABLE_NAME, fun ?MODULE:decode/1, fun ?MODULE:decode_value/1),
    Data = sora_runtime:decode_map_table(Rows, fun(Row) -> maps:get('id', Row) end),
    Keys = [maps:get('id', Row) || Row <- Rows],
    #{
        keys => Keys,
        data => Data
    }.
-spec get(integer(), table()) -> t() | undefined.
get(Key, Table) ->
    maps:get(Key, maps:get(data, Table), undefined).

-spec rows(table()) -> #{integer() => complex_condition_group_entry:t()}.
rows(Table) ->
    maps:get(data, Table).

-spec keys(table()) -> [integer()].
keys(Table) ->
    maps:get(keys, Table).

-spec ordered_rows(table()) -> [t()].
ordered_rows(Table) ->
    Data = maps:get(data, Table),
    [maps:get(Key, Data) || Key <- keys(Table), maps:is_key(Key, Data)].
