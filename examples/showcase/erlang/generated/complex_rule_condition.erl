...
-type t() :: #{
    'id' := integer(),
    'rule_id' := integer(),
    'value' := event_condition:t()
}.

-type table() :: map().
-define(TABLE_NAME, <<"ComplexRuleCondition">>).

-spec decode(sora_runtime:reader()) -> {t(), sora_runtime:reader()}.
decode(Reader0) ->
    {Id, Reader1} = (fun sora_runtime:read_i32/1)(Reader0),
    {RuleId, Reader2} = (fun sora_runtime:read_i32/1)(Reader1),
    {Value, Reader3} = (fun event_condition:decode/1)(Reader2),
    {#{
        'id' => Id,
        'rule_id' => RuleId,
        'value' => Value
...
decode_value(Value) ->
    ...
    #{
        ...
        'rule_id' => sora_runtime:expect_integer(sora_runtime:value_get(<<"rule_id">>, Obj)),
        'value' => event_condition:decode_value(sora_runtime:value_get(<<"value">>, Obj))
    }.
...
-spec rows(table()) -> #{integer() => complex_rule_condition:t()}.
rows(Table) ->
    maps:get(data, Table).
