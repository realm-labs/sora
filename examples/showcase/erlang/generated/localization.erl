%% This file was generated by Sora. Do not edit manually.

-module(localization).
-export([decode/1, decode_value/1, decode_table/1, rows/1
    , get/2, keys/1, ordered_rows/1
]).
-export_type([t/0, table/0]).

-type t() :: #{
    'key' := binary(),
    'zh_cn' := binary(),
    'en_us' := binary(),
    'note' := binary() | undefined
}.

-type table() :: map().
-define(TABLE_NAME, <<"Localization">>).

-spec decode(sora_runtime:reader()) -> {t(), sora_runtime:reader()}.
decode(Reader0) ->
    {Key, Reader1} = (fun sora_runtime:read_string/1)(Reader0),
    {ZhCn, Reader2} = (fun sora_runtime:read_string/1)(Reader1),
    {EnUs, Reader3} = (fun sora_runtime:read_string/1)(Reader2),
    {Note, Reader4} = (fun(Reader) -> sora_runtime:read_optional(fun sora_runtime:read_string/1, Reader) end)(Reader3),
    {#{
        'key' => Key,
        'zh_cn' => ZhCn,
        'en_us' => EnUs,
        'note' => Note
    }, Reader4}.

-spec decode_value(map()) -> t().
decode_value(Value) ->
    Obj = sora_runtime:expect_map(Value),
    #{
        'key' => sora_runtime:expect_binary(sora_runtime:value_get(<<"key">>, Obj)),
        'zh_cn' => sora_runtime:expect_binary(sora_runtime:value_get(<<"zh_cn">>, Obj)),
        'en_us' => sora_runtime:expect_binary(sora_runtime:value_get(<<"en_us">>, Obj)),
        'note' => (fun(OptionalValue) -> case OptionalValue of undefined -> undefined; _ -> sora_runtime:expect_binary(OptionalValue) end end)(sora_runtime:value_get(<<"note">>, Obj))
    }.

-spec decode_table(map()) -> table().
decode_table(Bundle) ->
    Rows = sora_runtime:decode_table(Bundle, ?TABLE_NAME, fun ?MODULE:decode/1, fun ?MODULE:decode_value/1),
    Data = sora_runtime:decode_map_table(Rows, fun(Row) -> maps:get('key', Row) end),
    Keys = [maps:get('key', Row) || Row <- Rows],
    #{
        keys => Keys,
        data => Data
    }.
-spec get(binary(), table()) -> t() | undefined.
get(Key, Table) ->
    maps:get(Key, maps:get(data, Table), undefined).

-spec rows(table()) -> #{binary() => localization:t()}.
rows(Table) ->
    maps:get(data, Table).

-spec keys(table()) -> [binary()].
keys(Table) ->
    maps:get(keys, Table).

-spec ordered_rows(table()) -> [t()].
ordered_rows(Table) ->
    Data = maps:get(data, Table),
    [maps:get(Key, Data) || Key <- keys(Table), maps:is_key(Key, Data)].
