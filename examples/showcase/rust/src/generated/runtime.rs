...
const SORA_BUNDLE_VERSION: u32 = 3;
...
fn map_from_pairs<K, V, E>(pairs: Vec<(K, V)>) -> Result<std::collections::HashMap<K, V>, E>
where
    K: Eq + std::hash::Hash,
    E: serde::de::Error,
{
    let mut values = std::collections::HashMap::with_capacity(pairs.len());
    for (key, value) in pairs {
        if values.insert(key, value).is_some() {
            return Err(E::custom("duplicate map key"));
        }
    }
    Ok(values)
}

pub mod serde_map_pairs {
        ...
        V: serde::Deserialize<'de>,
    {
        ...
        super::map_from_pairs(pairs)
    }
}

pub mod serde_optional_map_pairs {
    pub fn serialize<S, K, V>(
    ...
    {
        match value {
            Some(value) => super::serde_map_pairs::serialize(value, serializer),
    ...
    pub fn deserialize<'de, D, K, V>(
        deserializer: D,
    ) -> Result<Option<std::collections::HashMap<K, V>>, D::Error>
    where
        D: serde::Deserializer<'de>,
        K: serde::Deserialize<'de> + Eq + std::hash::Hash,
        V: serde::Deserialize<'de>,
    {
        let pairs = <Option<Vec<(K, V)>> as serde::Deserialize>::deserialize(deserializer)?;
        pairs.map(super::map_from_pairs).transpose()
    }
}
