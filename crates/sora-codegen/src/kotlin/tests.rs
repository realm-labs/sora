...
fn generates_rust_and_kotlin_files() {
    ...
    assert!(rust_item.contains("pub item_type: crate::item_type::ItemType"));
    assert!(rust_item.contains("pub start_at: std::time::SystemTime"));
    assert!(rust_item.contains("#[serde(with = \"crate::runtime::serde_system_time_millis\")]"));
    assert!(rust_item.contains("pub action: crate::action::Action"));
    ...
    assert!(rust_item.contains("impl crate::runtime::SoraDecode for Item"));
    ...
    assert!(rust_action.contains("impl crate::runtime::SoraDecode for Action"));
    ...
    assert!(rust_item.contains("by_item_type: SoraMap<crate::item_type::ItemType, Vec<i32>>"));
    assert!(rust_item.contains("pub const INFO: crate::SoraTableInfo"));
    assert!(rust_item.contains("shape: crate::SoraTableShape::Keyed"));
    assert!(rust_item.contains("primary_key: Some(crate::SoraKeyInfo"));
    assert!(rust_item.contains("impl crate::ErasedSoraTable for ItemTable"));
    assert!(rust_item.contains("impl crate::SoraKeyedTable for ItemTable"));
    ...
    assert!(rust_item.contains(
        "pub fn find_by_item_type(&self, item_type: crate::item_type::ItemType) -> impl Iterator<Item = &Item>"
    ));
...
fn generators_apply_custom_type_mappings() {
    ...
    assert!(rust_spawn.contains(
        "position: Vector3::from_sora(<crate::vec3::Vec3 as crate::runtime::SoraDecode>::decode(reader)?)"
    ));
    assert!(rust_spawn.contains(
        "values.push(Vector3::from_sora(<crate::vec3::Vec3 as crate::runtime::SoraDecode>::decode(reader)?));"
    ));
