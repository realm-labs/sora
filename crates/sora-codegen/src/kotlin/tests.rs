...
fn generates_rust_and_kotlin_files() {
    ...
    assert!(rust_item.contains("pub item_type: super::item_type::ItemType"));
    ...
    assert!(rust_item.contains("pub action: super::action::Action"));
    ...
    assert!(rust_item.contains("pub struct ItemId("));
    assert!(rust_item.contains("pub id: ItemId"));
    assert!(rust_item.contains("rows: SoraMap<ItemId, Item>"));
    assert!(rust_item.contains("keys: Vec<ItemId>"));
    assert_contains_before(
        &rust_item,
        "keys: Vec<ItemId>",
        "rows: SoraMap<ItemId, Item>",
    );
    assert!(rust_item.contains("by_name: SoraMap<String, ItemId>"));
    assert!(rust_item.contains("by_item_type: SoraMap<super::item_type::ItemType, Vec<ItemId>>"));
    ...
    assert!(rust_item.contains("pub fn get(&self, key: &ItemId) -> Option<&Item>"));
    assert!(rust_item.contains("pub fn keys(&self) -> &[ItemId]"));
    ...
    assert!(rust_item.contains(
        "pub fn find_by_item_type(&self, item_type: super::item_type::ItemType) -> impl Iterator<Item = &Item>"
    ));
...
fn generators_apply_custom_type_mappings() {
    ...
    assert!(rust_spawn.contains(
        "position: Vector3::from_sora(<super::vec3::Vec3 as super::runtime::SoraDecode>::decode(reader)?)"
    ));
    assert!(rust_spawn.contains(
        "values.push(Vector3::from_sora(<super::vec3::Vec3 as super::runtime::SoraDecode>::decode(reader)?));"
    ));
