...
mod tests {
    ...
    fn generates_csharp_java_and_go_files() {
        ...
        assert!(
            csharp_item.contains("private readonly Dictionary<ItemType, List<Item>> byItemType")
        );
        assert!(csharp_item.contains("public Item? GetByName(string name)"));
        assert!(
            csharp_item.contains("public IReadOnlyList<Item> FindByItemType(ItemType itemType)")
        );
