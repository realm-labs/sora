...
impl CodeGenerator for GodotCodeGenerator {
    fn generate(&self, ir: &ConfigIr, out_dir: &Path) -> Result<()> {
        if ir.codegen.godot.runtime_format != RuntimeFormatIr::Json {
            return Err(SoraError::InvalidSchema(format!(
                "godot codegen runtime_format `{}` is not implemented yet; supported runtime_format: json",
                crate::generator::runtime_format_name(ir.codegen.godot.runtime_format)
            )));
        }
...
"#,
        ...
        GodotCodeGenerator.generate(&ir, &out).unwrap();
