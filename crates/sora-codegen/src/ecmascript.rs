...
use crate::{
    model::{
        ...
        BaseUnion, BaseUnionVariant, schema_flat_pascal_name, schema_local_name,
    },
    options::EnumRepr,
...
pub fn ecmascript_barrels(modules: &[String], import_ext: &str) -> Vec<(String, String)> {
    let mut leaves = BTreeMap::<String, BTreeSet<String>>::new();
    let mut children = BTreeMap::<String, BTreeSet<String>>::new();

    for module in modules {
        let (namespace, leaf) = module.rsplit_once('/').unwrap_or(("", module.as_str()));
        leaves
    ...
    namespaces
        ...
        .map(|namespace| {
            ...
            if namespace.is_empty() {
                output.push_str(&format!("export * from \"./sora_runtime{import_ext}\";\n"));
                output.push_str(&format!("export * from \"./sora_config{import_ext}\";\n"));
            }
            if let Some(namespace_leaves) = leaves.get(&namespace) {
                for leaf in namespace_leaves {
                    output.push_str(&format!("export * from \"./{leaf}{import_ext}\";\n"));
                }
            }
            if let Some(namespace_children) = children.get(&namespace) {
                for child in namespace_children {
                    output.push_str(&format!(
                        "export * as {child} from \"./{child}/index{import_ext}\";\n"
                    ));
                }
            }
        ...
        Self {
            ...
            enums: model
                ...
                .map(|item| EcmaScriptEnum {
                    ...
                    runtime_module: ecmascript_runtime_module(&item.namespace),
...
fn ecmascript_union(
    ...
    let runtime_module = ecmascript_runtime_module(&union.namespace);
    let owner = union.qualified_name.clone();
    let variants = union
        .variants
        .into_iter()
        .map(|variant| ecmascript_variant(ir, &owner, variant, mapper))
        .collect::<Vec<_>>();
    let custom_imports =
...
fn ecmascript_variant(
    ...
    let fields = variant
        .fields
        .into_iter()
        .map(|field| ecmascript_field(ir, owner, field, mapper))
        .collect::<Vec<_>>();
...
fn ecmascript_record(
    ...
    let runtime_module = ecmascript_runtime_module(&record.namespace);
    let owner = record.qualified_name.clone();
    let fields = record
        .fields
        .into_iter()
        .map(|field| ecmascript_field(ir, &owner, field, mapper))
...
fn ecmascript_table(
    ...
    let key_type = table
        .key_field
        .as_ref()
        .map(|field| mapper.type_name(&owner, &field.ty));
    let key_field_name = table
        .key_field
        .as_ref()
        .map(|field| field.camel_name.clone());
    let config_name = schema_flat_pascal_name(&table.name);
    let camel_name = config_name.to_lower_camel_case();
    ...
    EcmaScriptTable {
        ...
        unique_indexes: table
            .unique_indexes
            .into_iter()
            .map(|index| ecmascript_index(ir, &owner, index, mapper))
            .collect(),
...
fn ecmascript_index(
    ...
    let key_type = mapper.type_name(owner, &index.field.ty);
    EcmaScriptIndex {
...
fn ecmascript_field(
    ...
    EcmaScriptField {
        ...
        type_name: mapper.type_name(owner, &field.ty),
        decode: ecmascript_decode_expr(ir, owner, &field.ty, mapper),
        value_decode: ecmascript_value_decode_expr(ir, owner, &field.ty, "__VALUE__", mapper),
        collect_text_keys,
...
fn ecmascript_import(import: BaseImport) -> EcmaScriptImport {
    let alias = schema_flat_pascal_name(&import.absolute_module.replace('/', "."));
    EcmaScriptImport {
...
fn ecmascript_runtime_module(namespace: &str) -> String {
    if namespace.is_empty() {
        "./sora_runtime".to_owned()
    } else {
        format!("{}sora_runtime", "../".repeat(namespace.split('.').count()))
    }
}

fn ecmascript_named_name(owner: &str, name: &str) -> String {
    if owner == name {
...
impl<'a> EcmaScriptTypeMapper<'a> {
    ...
    fn type_name(&self, owner: &str, ty: &TypeIr) -> String {
        ...
        match ty {
            ...
            TypeIr::Text => "TextKey".to_owned(),
            TypeIr::Enum(name) | TypeIr::Struct(name) | TypeIr::Union(name) => {
                ecmascript_named_name(owner, name)
            }
            TypeIr::List(element) | TypeIr::Set(element) | TypeIr::Array { element, .. } => {
                format!("readonly {}[]", self.array_element_type(owner, element))
            }
            TypeIr::Map { key, value } => {
                format!(
                    "ReadonlyMap<{}, {}>",
                    self.type_name(owner, key),
                    self.type_name(owner, value)
                )
            }
            TypeIr::Ref { table, field } => ref_type(self.ir, table, field)
                .map(|ty| self.type_name(owner, ty))
                .unwrap_or_else(|| "number".to_owned()),
            TypeIr::Optional(element) => self
                .mapping(element)
                .and_then(|mapping| mapping.nullable_type_name)
                .unwrap_or_else(|| format!("{} | undefined", self.type_name(owner, element))),
        }
    }

    fn array_element_type(&self, owner: &str, ty: &TypeIr) -> String {
        let name = self.type_name(owner, ty);
        if name.contains(" | ") || name.starts_with("readonly ") {
            format!("({})", schema_local_name(&name))
        } else {
            name
        }
    }
...
fn ecmascript_decode_expr(
    ...
    match ty {
        ...
        TypeIr::Enum(name) | TypeIr::Struct(name) | TypeIr::Union(name) => mapper.wrap_decode(
            ty,
            format!("decode{}(reader)", ecmascript_named_name(owner, name)),
        ),
        TypeIr::List(element) | TypeIr::Set(element) | TypeIr::Array { element, .. } => {
            format!(
                "reader.readList(() => {})",
                ecmascript_decode_expr(ir, owner, element, mapper)
            )
        }
        TypeIr::Map { key, value } => format!(
            "reader.readMap(() => {}, () => {})",
            ecmascript_decode_expr(ir, owner, key, mapper),
            ecmascript_decode_expr(ir, owner, value, mapper)
        ),
        TypeIr::Ref { table, field } => ref_type(ir, table, field)
            .map(|ty| ecmascript_decode_expr(ir, owner, ty, mapper))
...
fn ecmascript_value_decode_expr(
    ...
    match ty {
        ...
        TypeIr::Enum(name) | TypeIr::Struct(name) | TypeIr::Union(name) => mapper
            .wrap_value_decode(
                ty,
                format!("decode{}Value({value})", ecmascript_named_name(owner, name)),
            ),
        TypeIr::List(element) | TypeIr::Set(element) | TypeIr::Array { element, .. } => {
            let item_decode = ecmascript_value_decode_expr(ir, owner, element, "item", mapper);
        ...
        TypeIr::Map {
            ...
            let key_decode = ecmascript_value_decode_expr(ir, owner, key, "item", mapper);
            let value_decode = ecmascript_value_decode_expr(ir, owner, element, "item", mapper);
            format!("{value}.asMap((item) => {key_decode}, (item) => {value_decode})")
        }
        TypeIr::Ref { table, field } => ref_type(ir, table, field)
            .map(|ty| ecmascript_value_decode_expr(ir, owner, ty, value, mapper))
            .unwrap_or_else(|| format!("{value}.asInt()")),
        TypeIr::Optional(element) => {
            let item_decode = ecmascript_value_decode_expr(ir, owner, element, value, mapper);
...
fn ecmascript_collect_text_keys(
    ...
    match ty {
        ...
        TypeIr::Optional(element) => {
            let inner = ecmascript_collect_text_keys(ir, owner, element, "item", mapper);
            if inner.is_empty() {
                String::new()
            } else {
        ...
        TypeIr::Map {
            ...
            let key_inner = ecmascript_collect_text_keys(ir, owner, key, "mapKey", mapper);
            let value_inner = ecmascript_collect_text_keys(ir, owner, element, "item", mapper);
        ...
        TypeIr::Ref { table, field } => ref_type(ir, table, field)
            .map(|ty| ecmascript_collect_text_keys(ir, owner, ty, value, mapper))
            .unwrap_or_default(),
        TypeIr::Bool
        | TypeIr::I8
        | TypeIr::U8
